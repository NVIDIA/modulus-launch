...
class Inferencer(Trainer):
    ...
    def __init__(self, params, world_rank):
        ...
        # set amp_parameters
        self.amp_enabled = hasattr(params, "amp_enabled") and (
            params.amp_mode != "none"
        )
        self.amp_dtype = (
            None
            if not hasattr(params, "amp_enabled")
            else (
                torch.float16
                if (params.amp_mode == "fp16")
                else (torch.bfloat16 if (params.amp_mode == "bf16") else None)
            )
        )
    ...
    def inference_epoch(self):
        """
        Runs the model in autoregressive inference mode on the entire validation dataset. Computes metrics and scores the model against IFS.
        """
    ...
    def score_model(self):
        ...
        if self.params.log_to_screen:
            ...
            self.rank_zero_logger.info("Starting Scoring...")
