...
def main(cfg: DictConfig) -> None:
    ...
    for data in dp:
        ...
        for i in range(num_steps):
            ...
            ax[1].imshow(data[0]["latlon"][0, 0, :, :].detach().cpu().numpy(), origin="lower")
            ax[1].set_title("latlon")
            ax[2].imshow(data[0]["cos_latlon"][0, 0, :, :].detach().cpu().numpy(), origin="lower")
            ax[2].set_title("cos_latlon")
            ax[3].imshow(data[0]["cos_zenith"][0, i, 0, :, :].detach().cpu().numpy(), origin="lower")
