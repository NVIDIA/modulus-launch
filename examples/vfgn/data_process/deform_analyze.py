...
def plot_temperature_curve2(key_list, temp_list):
    ...
    fig.savefig('temperature_profile_complete.png',
                format='png',
                dpi=100,
                bbox_inches='tight')
...
# plot
def plot_p_deform(temp_list, key_list, stage_keys,
                  del_u, del_v, del_w, pid=0):
...
def plot_p_deform_simple(build_name, temp_list, key_list,
                         del_u, del_v, del_w, pid=0):
    # sol_list = [i for i in range(len(temp_list))]
    sol_list = key_list
...
def read_solutions_data_temp_anchor(raw_data_path, build_name, start_temp=500, end_temp=2000):
    """ 1st version """
    ...
    for solution_idx, solution_path in enumerate(solution_list):
        ...
        solution_temp = rawdata2tfrecord.get_solution_temperature_customer(solution_path,
                                                                           dict_sol_time, temp_curve_list)
    ...
    plot_p_deform_simple(build_name, temp_list, key_list=key_list,
                         del_u=del_u, del_v=del_v, del_w=del_w, pid=read_point_id)
...
def main(argv):
    ...
    for build_name in build_list:
        key_list, temp_list = read_solutions_data_temp_anchor(os.path.join(raw_data_dir, build_name),build_name,
