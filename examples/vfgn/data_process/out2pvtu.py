...
def pv_get_data_position(data):
    ...
    print("complete points shape: ", points.shape)   # i.e. all points shape:  (134464, 3)
...
def update_points(basemesh_points, metadata, new_pos,
                  position_ids_dict, index_list):
    """
...
def write_output(basemesh_path, new_uvw_array, end_inference_index, outPath):
    ...
    predicted_file_path=os.path.join(outPath,"predicted-displacement-{}.vtu".format(end_inference_index))
...
def save_volume_deformation(basemesh_path, new_uvw_array, end_inference_index, outPath, core_id):
    uvw_vtk_array = vtk.vtkDoubleArray()
    uvw_vtk_array.SetNumberOfComponents(3)
    uvw_vtk_array.SetName("displacement_U")
    # add uvw-displacement values to the array
    ...
    predicted_vtu_solution.SetCells(mesh_vtu_file.GetCellTypesArray(), mesh_vtu_file.GetCells())
    ...
    predicted_file_path=os.path.join(outPath,"predicted-displacement-{}-{}.vtu".format(str(core_id).rjust(4, '0'), end_inference_index))
    predicted_vtu_solution_writer.SetFileName(predicted_file_path)
    predicted_vtu_solution_writer.Write()
...
def save_volume_deformation_master_record(outPath, vtu_list, end_inference_index):
    ...
    master_path = os.path.join(outPath, "predicted-displacement-{}.pvtu".format(end_inference_index))
    ...
    master_file.write("<VTKFile type=\"PUnstructuredGrid\" version=\"0.1\" byte_order=\"LittleEndian\">\n")
    master_file.write("<PUnstructuredGrid GhostLevel=\"0\">\n")
    master_file.write("  <PPointData Scalars=\"scalars\">\n")
    master_file.write("    <PDataArray type=\"Float64\" Name=\"displacement_U\" NumberOfComponents=\"3\" format=\"ascii\"/>\n")
    ...
    for i, vtu_name in enumerate(vtu_list):
        ...
        master_file.write("  <Piece Source=\""+ vtu_name +  "\"/>\n")    # displacement-0000-1505.vtu
...
def post_process(raw_data_path, metadata, example_rollout,
                 end_inference_index, outPath):
    """
    Args:
    ...
    print(example_rollout['predicted_rollout'].shape) # i.e. (47, 21969, 3): (predicted_time_steps, num_nodes, xyz-dim)
    ...
    # print("solution list: ", solution_list, len(solution_list))
    # exit()
    ...
    assert os.path.exists(basemesh_path), print(f"basemesh does not exist: {basemesh_path}")
    ...
    new_uvw_array, _ = update_points(basemesh_points, metadata, example_rollout['predicted_rollout'],
                                  position_ids_dict, index_list)

    # write to a new vtu file
    predicted_file_path=write_output(basemesh_path, new_uvw_array,end_inference_index,outPath)
...
if __name__ == '__main__':
    ...
    # print("example_rollout: ", example_rollout)

    post_process(raw_data_path, example_rollout['metadata'], example_rollout,
                 end_inference_index, outPath)
