        ...
        if FLAGS.loss.startswith('anchor'):
            ...
            for idx_i in range(len(random_pids)):
                for idx_j in range(idx_i, len(random_pids)):
                    ...
                    corr_gt = torch.nn.functional.cosine_similarity(target_acceleration[i], target_acceleration[j],
                                                                    dim=0)
        ...
        for features, targets in tqdm(dataset):
            if loaded is False:
                ...
                if global_context is None:
                    ...
                    read_step_context = global_context[:-1]
                    zero_pad = torch.zeros([38-read_step_context.shape[0]-1, 1], dtype=features['step_context'].dtype).to(device)
