...
class Inferencer():
    ...
    def __init__(self, params, world_rank):
        ...
        self.iters = 0
        self.startEpoch = 0
        self.restore_checkpoint(params.pretrained_checkpoint_path, distributed = dist.is_initialized())
    ...
    def inference_one_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            eval_steps = 0
            for data in tqdm(self.valid_dataloader, desc="Inference progress", disable=not self.params.log_to_screen):
    ...
    def save_checkpoint(self, checkpoint_path, model=None):
        ...
        torch.save({'iters': self.iters, 'epoch': self.epoch, 'model_state': model.state_dict(),
                    'optimizer_state_dict': self.optimizer.state_dict(), 'scheduler_state_dict': self.scheduler.state_dict()}, checkpoint_fname)
    ...
    def restore_checkpoint(self, checkpoint_path, distributed=True):
        ...
        checkpoint = torch.load(checkpoint_fname, map_location='cuda:{}'.format(self.device.index) if self.device.type == 'cuda' else 'cpu')
